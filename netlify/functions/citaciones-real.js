...
function parseHtmlData(html) {
      ...
      try {
            ...
            const searchForCitaciones = (obj, path = '') => {
              ...
              Object.keys(obj).forEach(key => {
                ...
                if (key === 'CITACIONES' && value) {
                  console.log(`Found CITACIONES data at ${path}.${key}:`, typeof value, Array.isArray(value) ? value.length : 'not array');
                  
                  if (Array.isArray(value)) {
                    value.forEach((citacion, index) => {
                      if (citacion && typeof citacion === 'object') {
                        console.log(`Processing citacion ${index}:`, Object.keys(citacion));
                        
                        // Extract commission information with correct field names
                        let titulo = citacion.COMINOMBRE || citacion.TITULO || citacion.titulo || citacion.COMISION || citacion.comision || '';
                        ...
                        if (titulo || details.length > 0) {
                          citaciones.push({
                            ...
                            time: horario,
                            subject: materia,
                            source: 'Next.js CITACIONES data'
                          });
                        }
                      }
                    });
                  } else if (typeof value === 'object') {
                    // Single citacion object with correct field names
                    let titulo = value.COMINOMBRE || value.TITULO || value.titulo || value.COMISION || value.comision || '';
                    ...
                    if (titulo || details.length > 0) {
                      citaciones.push({
                        ...
                        time: horario,
                        subject: materia,
                        source: 'Next.js CITACIONES object'
                      });
                    }
                  }
                }
                ...
                if (typeof value === 'string') {
                      ...
                      comisionMatches.forEach(match => {
                        ...
                        if (!citaciones.find(c => c.title === match.trim())) {
                          citaciones.push({
                            ...
                            time: obj.HORARIO || '',
                            subject: obj.MATERIA || '',
                            source: 'Next.js commission text parsing'
                          });
                        }
                      });
                    }
                  ...
                  if (value.includes('julio') && value.includes('Comisión')) {
                    console.log(`Found potential date pattern with commission: ${value.substring(0, 200)}`);
                  }
        ...
        lines.forEach(line => {
          if (line.includes('Comisión de') && (line.includes('Hacienda') || line.includes('Medio Ambiente') || line.includes('julio'))) {
            console.log(`Found relevant line: ${line.substring(0, 200)}`);
  ...
  if (citaciones.length === 0) {
    console.log('No detailed JavaScript data found, falling back to HTML extraction...');
      ...
      if (matches) {
        matches.forEach(match => {
          ...
          citaciones.push({
            ...
            source: 'Pattern matching extraction'
          });
        });
      }
    });
    ...
    if (bodyText.includes('Hacienda') && bodyText.includes('10:30')) {
      citaciones.push({
        ...
        source: 'Keyword detection enhanced'
      });
    }
  ...
  uniqueCitaciones.forEach((citacion, i) => {
    console.log(`${i + 1}. ${citacion.title} - ${citacion.time} - ${citacion.location}`);
  });
